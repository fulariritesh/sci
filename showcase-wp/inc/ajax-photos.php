<?php
add_action("wp_ajax_sci_add_photos", "sci_add_photos");
function sci_add_photos() {
	...
	$upload_id = NULL;
   	...
   	var_dump($_REQUEST);
	/*   	if(isset($_REQUEST["photos"]) && isset($_REQUEST["index"])) {
		...
		if((!is_wp_error($upload_id)) || ($upload_id !== 0)){
			...
			if(have_rows('sci_user_photoss', 'user_' . $user_id)){
				$row = array('sci_user_photos' => $upload_id);
				$success = update_row('sci_user_photoss', $index, $row, 'user_'.$user_id);
			}else{
				$row = array('sci_user_photos' => $upload_id);
				$success = add_row('sci_user_photoss', $row, 'user_'.$user_id);
			}
...
add_action("wp_ajax_sci_delete_photos", "sci_delete_photos");
function sci_delete_photos() {
	$user_id = get_current_user_id();
   	...
   	if(!empty($_REQUEST["index"])) {
        $index = intval($_REQUEST["index"]);     
        $success = delete_row('sci_user_photoss', $index, 'user_'.$user_id);
        if($success){
            echo json_encode(array('status' => 'success','msg' =>  'Success.' ));
            exit();	
        }else{
